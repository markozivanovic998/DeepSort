...
def process_straight_line(track_id, current_pos, line_coord, line_type, target_direction_indicator, counter_list, person_state_dict, passed_any_line_set):
    ...
    if line_type == 'down': 
        current_zone = 'above' if current_pos < line_coord else 'below'
    elif line_type == 'up': 
        current_zone = 'below' if current_pos > line_coord else 'above'
    elif line_type == 'right': 
        current_zone = 'left' if current_pos < line_coord else 'right'
    elif line_type == 'left': 
        current_zone = 'right' if current_pos > line_coord else 'left'
    
    if track_id not in person_state_dict: 
        person_state_dict[track_id] = {}
    
    if line_type not in person_state_dict[track_id]: 
...
def process_diagonal_line(track_id, cx, cy, start_diag, end_diag, line_name, counter_list, person_state_dict, passed_any_line_set):
    ...
    if line_name not in person_state_dict[track_id]: 
